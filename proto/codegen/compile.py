...
# The spec lives in eureka/proto/, a sibling of each firmware project. Headers are
# generated into the build dir and included as "proto/Packet_<Name>.h".
spec_path = os.path.realpath(os.path.join(env.subst("$PROJECT_DIR"), "..", "proto"))
generated_path = os.path.join(env.subst("$BUILD_DIR"), "generated")
env.Append(CPPPATH=[generated_path])
...
# Out: dict of {}
def create_packet_headers(packet_list, payloads, enums, config, build_path):
    packet_structs = {}
...
def make_headers():
    ...
    packets_for_packet_group = get_packet_hierarchy(spec_path)
    
    payloads, enums = get_payloads_and_enums(spec_path)

    print("Packet Spec Config: " + os.path.join(spec_path, "config_" + system + ".jsonc"))
    config = get_config("config_" + system + ".jsonc", spec_path)

    packet_structs = {}

    create_packet_headers(packets_for_packet_group, payloads, enums, config, os.path.join(generated_path, "proto"))
